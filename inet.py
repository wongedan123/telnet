...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('list.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check list.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('w0n63d4n Connecting'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('w0n63d4n Connected ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
